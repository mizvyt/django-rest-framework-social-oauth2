...
def _authorize_user(request, fn, *args, **kwargs):
    try:
        user = fn(*args, **kwargs)
...
class ConvertAuthorizationCodeGrant(RefreshTokenGrant):
    ...
    def validate_token_request(self, request):
        ...
        request.state = backend.get_or_create_state()

        backend.data['code'] = request.code
        backend.data['state'] = request.state

        request.user = _authorize_user(request, backend.auth_complete)
...
class ConvertAccessTokenGrant(RefreshTokenGrant):
    ...
    def validate_token_request(self, request):
        ...
        request.user = _authorize_user(request, backend.do_auth, access_token=request.token)
        log.debug('Authorizing access to user %r.', request.user)
